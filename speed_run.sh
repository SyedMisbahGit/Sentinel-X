#!/bin/bash
# ==============================================================================
# SENTINEL-X: SPEED RUN (TRIAGE MODE)
# Fast detection of Critical CVEs and Reflected Parameters.
# Timeout: 3s | Scope: High Impact Only
# ==============================================================================
TARGET=$1
...
if [[ ! -d "$RDIR" ]]; then 
    echo "No discovery data found. Run sentinel_x.sh first."
    exit 1
fi
...
# 1. FAST NUCLEI
echo -e "\n\033[1;32m[+] Checking for Critical CVEs & Panels (Timeout: 3s)...\033[0m"
...
# 2. INSTANT XSS REFLECTION
echo -e "\n\033[1;32m[+] Checking for Parameter Reflection...\033[0m"
...
if [[ -s "$RDIR/xss_reflected.txt" ]]; then
    ...
    cat "$RDIR/xss_reflected.txt" | head -n 10
    echo "... (See full list in reports)"
fi
